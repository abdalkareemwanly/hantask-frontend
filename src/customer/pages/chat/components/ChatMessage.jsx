...
function CustomerChatMessage({ type, message, date, isFirst, name, image, file }) {
  try {
    return (
      <>
        <div className={`w-[100%] flex flex-${type == "received" ? "row-reverse" : "row"} gap-[10px] self-${type == "received" ? "end" : "start"} mt-${isFirst ? "[10px]" : "0"}`}>
          {isFirst && <img src={image} className="w-[30px] h-[30px] rounded-full" />}
          <div
          ...
          >
            ...
            <div>{message}</div>
            {file && (
              <a src={file} download className="text-primary-text opacity-[.7] hover:opacity-[1] duration-[500ms] flex items-center gap-[5px] text-[15px] mt-[20px] cursor-pointer">
                {" Download"}
                <FaFileDownload />
              </a>
            )}
