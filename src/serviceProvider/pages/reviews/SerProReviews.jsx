...
const getData = async () => {
  const res = await axiosClient.get("/seller/reviews");
  return res;
};

const deleteRow = async (id) => {
  const res = await axiosClient.get(`/seller/review/delete/${id}`);
  return res;
};
          ...
          <div className="flex  items-start gap-16">
            <div className="flex flex-col text-primary-text gap-[5px]">
              ...
              <div>
                ...
                <Link to={`buyer-profile/${row.review_to.id}`}>
                  {row.review_to.name}
                </Link>
              </div>
