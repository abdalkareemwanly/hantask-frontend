    ...
    return (
      <>
        <div className="sm:w-[90%] sm:mx-[auto] w-[96%] mx-[2%] my-[30px]">
          <h3 className="font-[600] text-[32px]">Available Jobs Sub Categories in {state.currentCategory?.name}</h3>
          <div className="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-[20px] mt-[30px]">
            {categories && subCategories && childCategories && state.currentCategory
              ? subCategories.map((item, index) => {
                  if (item.categoryName == state.currentCategory.name) return <Subcategory key={index} name={item.name} id={item.id} />;
                })
              : Array.from(Array(9).keys()).map((item, index) => {
                  return <CategoryLoader key={index} />;
                })}
          </div>
          <h3 className="font-[600] text-[32px] mt-[60px]">Available Jobs in {state.currentCategory?.name}</h3>
          {DEFAULT_DATA.length ? (
